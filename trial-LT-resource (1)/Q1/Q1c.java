...
public class Q1c {
    /*
     * Takes in an array of Human objects (Human.java is provided), and returns the
     * Human object
     * in the list that is the oldest (i.e. highest age). If more than one Human has
     * the same highest
     * age, this method should return the latter one in the list.
     *
     * e.g.: if the Human objects in humans have these ages: 34, 5, 6, 88, 9, 98,
     * 102, 38, 88
    ...
    public static Human getOldest(Human[] humans) {
        ...
        Human retHuman = humans[0];

        for (Human human : humans) {
            if (human.getAge() > retHuman.getAge()) {
                retHuman = human;
            }
        }
