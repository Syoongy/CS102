...
public class Q1f {
    /*
     * Takes in a 2D array of integers and returns a new 2D array with the sequences
     * changed.
     *
     * e.g. 1:if the input 2D array looks like this: {{1, 2}, {3, 4}, {5, 6}}, your
     * method will return a brand new 2D array consisting of the following ints:
     * {{1, 3, 5}, {2, 4, 6}}
    ...
    public static int[][] flip(int[][] numbers) {
        int[][] retArr = new int[numbers[0].length][numbers.length];

        for (int i = 0; i < numbers[0].length; i++) {
            for (int j = 0; j < numbers.length; j++) {
                retArr[i][j] = numbers[j][i];
            }
        }
