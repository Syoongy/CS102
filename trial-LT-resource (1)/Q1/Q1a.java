...
public class Q1a {
    ...
    /*
     * Takes in 2 arrays of chars, and returns a char array containing combined
     * elements from both inputs
     * e.g.: if a is {'a', 'p', 'p', 'l', 'e'};
     * b is {'1', '2', '3', '4', '5'};
     * then this method returns an array that contains the following chars:
     * {'a', '1', 'p', '2', 'p', '3', 'l', '4', 'e', '5'}
     * Note that the first char in the returned array always comes from a. This is
     * followed by
     ...
     *
     * This method should throw an IllegalInputException if a or b is null, or if
     * the number of
     * elements in a and b are not the same. Insert a relevant message into the
     * exception.
     */
    public static char[] combineTwoCharArrays(char[] a, char[] b) {
        ...
        if (a.length != b.length) {
            throw new IllegalInputException("The length of both arrays are different");
        }

        ArrayList<Character> retArrayList = new ArrayList<>();
        for (int i = 0; i < a.length; i++) {
            retArrayList.add(a[i]);
            retArrayList.add(b[i]);
        }
        char[] retArray = new char[retArrayList.size()];
