...
public class Q6PartC {
    ...
    public static String getFarmerDetails(CommunityGarden garden, int plotNumber) {
        ...
        String retString = "";
        Plot gardenPlot = garden.getPlot(plotNumber);

        if (gardenPlot == null) {
            retString = "N.A";
        } else {
            Farmer plotFarmer = gardenPlot.getFarmer();

            if (plotFarmer == null) {
                retString = "available";
            } else {
                retString = String.format("%s[%s]", plotFarmer.getName(),
                        Character.compare(plotFarmer.getGender(), 'M') == 0 ? "Male" : "Female");
            }

        }
        return retString; // added to make this code compile.
